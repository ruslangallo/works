...
(function($){
	$(document).ready(function(){
		var gallery 						= $('.gallery'),
			galleryItems 					= gallery.find('.gallery-item img'),
		...
		carousel.on('slid.bs.carousel', function(){
			var carousel					= $(this),
				gallery						= carousel.closest('.gallery'),
				inner						= gallery.find('.gallery-inner'),
				innerScroll					= inner.scrollLeft(),
				innerWidth					= inner.outerWidth(),
				item 						= gallery.find('.gallery-item'),
				id 							= carousel.find('.active').index(),
				el							= item.eq(id),
				elWidth						= el.outerWidth(),
				elFirstWidth				= item.eq(0).outerWidth(),
				innerPosition				= 0;
			...
			el.prevAll().each(function(){
				...
				innerPosition += elWidth;
			});
			innerPosition += elWidth - elFirstWidth;

			if(innerScroll >= innerPosition || innerPosition + elWidth >= innerScroll + innerWidth){
				inner.animate({
					scrollLeft: innerPosition
				});
			}
		});
	});
